...
char* adminNumber[]={"+79963825136","+79529220114"};//доверенные ном
int8_t i; 
...
SerialAT.print("AT+CLIP=1\r"); // устанавливаем определитель номера
 delay(500); //   пауза
...
void loop()
{
  ...
  ////////////////////////////////////////////////////
  if (!SerialAT.available())// если не данных от модуля SIM800l, то дальше не идем.
 return;
 
 char currSymb = SerialAT.read(); // записываем в переменую символы, которые получили от модуля.
 ...
 {
 if (currStr.startsWith("+CLIP:"))// если поступил звонок
 { 
